...
if (isset($_POST['loginbutton'])) {
    ...
    $sql = mysqli_query($conn, "SELECT * FROM courtstaffs where Username='$Username' and Pass='$Pass'");
    $row  = mysqli_fetch_array($sql);
    if (is_array($row)) {
        $_SESSION["uname"] = $row['Username'];
        $_SESSION["Pass"] = $row['Pass'];
        mysqli_query($conn, "UPDATE role SET id = 3 where name='role'");
...
<body>
    <div class="loginbox">
        ...
        <form action="courtlogin.php" method="post">
            <h1>COURT LOGIN</h1>
